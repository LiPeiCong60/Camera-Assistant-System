...
class MediaPipeVisionDetector(VisionDetector):
    ...
    def __init__(self, config: DetectionConfig) -> None:
        ...
        try:
            ...
            pose_options = vision.PoseLandmarkerOptions(
                ...
                num_poses=3,
                min_pose_detection_confidence=self._config.min_confidence,
                min_pose_presence_confidence=self._config.min_confidence,
            ...
            if self._config.enable_face_landmarks:
                ...
                face_options = vision.FaceLandmarkerOptions(
                    ...
                    running_mode=vision.RunningMode.IMAGE,
                    num_faces=1,
                    min_face_detection_confidence=self._config.min_confidence,
                    min_face_presence_confidence=self._config.min_confidence,
                    min_tracking_confidence=self._config.min_confidence,
    ...
    def _bbox_from_landmarks(self, landmarks, w: int, h: int) -> BBox | None:
        ...
        ys: list[int] = []
        for lm in landmarks:
            # 只检查坐标是否在图片范围内，不过滤 visibility
            # 这样可以确保包含所有检测到的关键点，包括下半身
            x = int(lm.x * w)
            y = int(lm.y * h)
            if 0 <= x < w and 0 <= y < h:
                xs.append(x)
...
class OpenCvHogPersonDetector:
    ...
    def detect_person_bbox(self, frame: np.ndarray) -> tuple[BBox | None, float]:
        ...
        x, y, w, h = boxes[idx]
        return BBox(x=int(x), y=int(y), w=int(w), h=int(h)), float(weights[idx])


class YoloOnlyVisionDetector(VisionDetector):
    """Fallback detector that tracks the primary person bbox via YOLO only."""
    ...
    def __init__(
        ...
        *,
        ...
        self._hog_fallback: OpenCvHogPersonDetector | None = None

    def detect(self, frame: np.ndarray) -> VisionResult:
        try:
            bbox, conf = self._yolo.detect_person_bbox(frame)
        except Exception:
            if self._hog_fallback is None:
                self._hog_fallback = OpenCvHogPersonDetector()
            bbox, conf = self._hog_fallback.detect_person_bbox(frame)
        if bbox is None:
            return VisionResult()
...
class AsyncDetector:
    ...
    Supports smart frame skipping with result interpolation.
    """

    def __init__(self, detector: VisionDetector, *, skip_frames: int = 0) -> None:
        ...
        self._skip_enabled = skip_frames > 0

    def submit(self, frame: np.ndarray) -> int:
        seq = self._next_seq
        self._next_seq += 1

        # Smart frame skipping: only submit every N frames
        if self._skip_enabled:
            self._frame_counter += 1
            if self._frame_counter % (self._skip_frames + 1) != 0:
                # Skip this frame, return last seq to indicate reuse
