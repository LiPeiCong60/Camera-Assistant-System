...
[项目说明](docs/项目说明.md) | [项目现状](docs/PROJECT_STATUS.md) | [接口契约](docs/接口契约.md) | [数据字典](docs/数据字典.md)
...
截至 2026-07-10，四端代码与 PostgreSQL 数据结构已经成形，手机拍摄、设备联动、云台控制、模板构图、AI 分析、历史记录和运营后台均有对应实现。项目适合本机、局域网和树莓派演示环境，真实硬件体验仍取决于摄像头、舵机、网络与模型服务配置。

当前 AI 调用主链路只实现 `openai_compatible` Provider；设备管理后端接口仍保留，但管理后台入口暂未启用。详细验证范围、已知限制和下一步计划见 [项目现状](docs/PROJECT_STATUS.md)。
