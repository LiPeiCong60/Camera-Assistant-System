...
sequenceDiagram
    ...
    D->>V: 打开 OpenCVVideoSource 或 mobile_push
    M->>D: WS /api/device/stream/mobile-ws 推送 NV21 帧
    D-->>M: WS /api/device/preview-ws 推送 JPEG 预览帧
...
| 分组 | 路径 | 作用 |
| --- | --- | --- |
| health/status | `/api/device/health`, `/api/device/status`, `/api/device/preview.jpg`, `/api/device/preview-ws` | 健康、状态、静态预览图、实时预览下行 |
...
| stream | `/api/device/stream/start`, `/api/device/stream/mobile-ws`, `/api/device/stream/frame` | 切换视频流地址；接收手机端 WebSocket 视频帧；保留 JPEG 回退调试接口 |

### 6.3 帧处理管线

```mermaid
flowchart TD
    A["OpenCVVideoSource / mobile_push 读帧"] --> B["AsyncDetector 异步检测"]
...
- 设备端是实时控制源，负责视频帧、手机推送帧、检测结果和云台状态。
- 手机端是操作入口，既连后端也连设备端。
- 管理后台只连后端，不直接控制设备。
...
6. 手机画面推送当前使用 WebSocket NV21 帧上行和 WebSocket JPEG 预览帧下行，不是 RTSP/WebRTC；移动端主路径优先覆盖 Android / 安卓模拟器。
7. 自动化测试主要在 Flutter 侧，后端和设备端缺少系统性测试。
